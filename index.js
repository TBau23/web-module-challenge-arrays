...
function is31Flavors(arr, testLength){
    if(arr.length === testLength){
        return true;
    }else{
        return false;
    }
...
function addFlavor(targetArr, newFlav){
    targetArr.unshift(newFlav);
...
function removeLastFlavor(targetArr){
    targetArr.pop();
    console.log(targetArr);
}
...
function getFlavorByIndex(targetArr, targetIndex){
...
function removeFlavorByName(targetArr, targetFlav){
    for(let i = 0; i < targetArr.length; i++){
        if(targetArr[i] === targetFlav){
...
// console.log(removeFlavorByName(originalFlavors, 'Cherry Macaron'));
...
function filterByWord(targetArr, keyword){
    filteredArray = []
    for(let i = 0; i < targetArr.length; i++){
        if(targetArr[i].includes(keyword)){
            filteredArray.unshift(targetArr[i]);
        }
    }
...
function getAverageWordLength(targetArr){
    totalSpaces = 0;
    for(let i = 0; i < targetArr.length; i++){
        if(targetArr[i][i] === " "){
            totalSpaces ++ ;
